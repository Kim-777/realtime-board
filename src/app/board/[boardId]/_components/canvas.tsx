...
export const Canvas = ({ boardId }: CanvasProps) => {
  ...
  const layerIdsToColorSelection = useMemo(() => {
    const newLayerIdsToColorSelection: Record<string, string> = {};

    for (const user of selections) {
      const [connectionId, selection] = user;

      for (const layerId of selection) {
        newLayerIdsToColorSelection[layerId] = connectionIdToColor(connectionId);
      }
    }

    return newLayerIdsToColorSelection;
  }, [selections]);
  ...
  useEffect(() => {
    function onKeyDown(this: Document, e: globalThis.KeyboardEvent) {
      switch (e.key) {
        // case "Backspace":
        //   deleteLayers();
        //   break;
    ...
    <main className='h-full w-full relative bg-neutral-100 touch-none'>
      ...
      >
        <g style={{ transform: `translate(${camera.x}px, ${camera.y}px)` }}>
          ...
          <CursorsPresence />
          {pencilDraft !== null && pencilDraft !== undefined && pencilDraft.length > 0 && (
